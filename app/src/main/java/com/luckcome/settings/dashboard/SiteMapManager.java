...
import com.luckcome.settings.search.IndexDatabaseHelper;
...
public class SiteMapManager {
    ...
    public static final String[] SITE_MAP_COLUMNS = {
            ...
            IndexDatabaseHelper.SiteMapColumns.PARENT_TITLE,
            IndexDatabaseHelper.SiteMapColumns.CHILD_CLASS,
            IndexDatabaseHelper.SiteMapColumns.CHILD_TITLE
    };
    ...
    synchronized void init(Context context) {
        ...
        while (sitemap.moveToNext()) {
            final SiteMapPair pair = new SiteMapPair(
                    sitemap.getString(sitemap.getColumnIndex(IndexDatabaseHelper.SiteMapColumns.PARENT_CLASS)),
                    sitemap.getString(sitemap.getColumnIndex(IndexDatabaseHelper.SiteMapColumns.PARENT_TITLE)),
                    sitemap.getString(sitemap.getColumnIndex(IndexDatabaseHelper.SiteMapColumns.CHILD_CLASS)),
                    sitemap.getString(sitemap.getColumnIndex(IndexDatabaseHelper.SiteMapColumns.CHILD_TITLE)));
            mPairs.add(pair);
        }
        sitemap.close();
        ...
        while (titleQuery.moveToNext()) {
            classToTitleMap.put(
                    titleQuery.getString(titleQuery.getColumnIndex(IndexDatabaseHelper.IndexColumns.CLASS_NAME)),
                    titleQuery.getString(titleQuery.getColumnIndex(IndexDatabaseHelper.IndexColumns.SCREEN_TITLE)));
        }
        titleQuery.close();
        ...
        for (DashboardCategory category : categories) {
            ...
            final String parentClass = DashboardFragmentRegistry.CATEGORY_KEY_TO_PARENT_MAP.get(category.key);
            if (parentClass == null) {
                continue;
            }
