...
public class CursorToSearchResultConverter {
    ...
    private SearchResult buildSingleSearchResultFromCursor(SiteMapManager sitemapManager,
        ...
        final int docId = cursor.getInt(DatabaseResultLoader.COLUMN_INDEX_ID);
        final String pkgName = cursor.getString(DatabaseResultLoader.COLUMN_INDEX_INTENT_ACTION_TARGET_PACKAGE);
        final String title = cursor.getString(DatabaseResultLoader.COLUMN_INDEX_TITLE);
        final String summaryOn = cursor.getString(DatabaseResultLoader.COLUMN_INDEX_SUMMARY_ON);
        final String className = cursor.getString(DatabaseResultLoader.COLUMN_INDEX_CLASS_NAME);
        final String key = cursor.getString(DatabaseResultLoader.COLUMN_INDEX_KEY);
        final String iconResStr = cursor.getString(DatabaseResultLoader.COLUMN_INDEX_ICON);
        final int payloadType = cursor.getInt(DatabaseResultLoader.COLUMN_INDEX_PAYLOAD_TYPE);
        final byte[] marshalledPayload = cursor.getBlob(DatabaseResultLoader.COLUMN_INDEX_PAYLOAD);
    ...
    private List<String> getBreadcrumbs(SiteMapManager siteMapManager, Cursor cursor) {
        final String screenTitle = cursor.getString(DatabaseResultLoader.COLUMN_INDEX_SCREEN_TITLE);
        final String screenClass = cursor.getString(DatabaseResultLoader.COLUMN_INDEX_CLASS_NAME);
