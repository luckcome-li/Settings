...
public class DatabaseIndexingManager {
    ...
    void updateDataInDatabase(SQLiteDatabase database,
        ...
        final String whereEnabled = IndexDatabaseHelper.IndexColumns.ENABLED + " = 1";
        final String whereDisabled = IndexDatabaseHelper.IndexColumns.ENABLED + " = 0";

        final Cursor enabledResults = database.query(IndexDatabaseHelper.Tables.TABLE_PREFS_INDEX, DatabaseResultLoader.SELECT_COLUMNS,
                whereEnabled, null, null, null, null);

        final ContentValues enabledToDisabledValue = new ContentValues();
        enabledToDisabledValue.put(IndexDatabaseHelper.IndexColumns.ENABLED, 0);
        ...
        while (enabledResults.moveToNext()) {
            ...
            packageName = enabledResults.getString(DatabaseResultLoader.COLUMN_INDEX_INTENT_ACTION_TARGET_PACKAGE);
            if (packageName == null) {
                packageName = mContext.getPackageName();
            }

            final String key = enabledResults.getString(DatabaseResultLoader.COLUMN_INDEX_KEY);
            ...
            if (packageKeys != null && packageKeys.contains(key)) {
                final String whereClause = IndexDatabaseHelper.IndexColumns.DOCID + " = " + enabledResults.getInt(DatabaseResultLoader.COLUMN_INDEX_ID);
                database.update(IndexDatabaseHelper.Tables.TABLE_PREFS_INDEX, enabledToDisabledValue, whereClause, null);
            }
        }
        enabledResults.close();

        final Cursor disabledResults = database.query(IndexDatabaseHelper.Tables.TABLE_PREFS_INDEX, DatabaseResultLoader.SELECT_COLUMNS,
                whereDisabled, null, null, null, null);

        final ContentValues disabledToEnabledValue = new ContentValues();
        disabledToEnabledValue.put(IndexDatabaseHelper.IndexColumns.ENABLED, 1);

        while (disabledResults.moveToNext()) {
            // Package name is the key for remote providers.
            // If package name is null, the provider is Settings.
            packageName = disabledResults.getString(DatabaseResultLoader.COLUMN_INDEX_INTENT_ACTION_TARGET_PACKAGE);
            if (packageName == null) {
                packageName = mContext.getPackageName();
            }

            final String key = disabledResults.getString(DatabaseResultLoader.COLUMN_INDEX_KEY);
            final Set<String> packageKeys = nonIndexableKeys.get(packageName);
            ...
            if (packageKeys != null && !packageKeys.contains(key)) {
                String whereClause = IndexDatabaseHelper.IndexColumns.DOCID + " = " + disabledResults.getInt(DatabaseResultLoader.COLUMN_INDEX_ID);
                database.update(IndexDatabaseHelper.Tables.TABLE_PREFS_INDEX, disabledToEnabledValue, whereClause, null);
            }
        }
    ...
    private void updateOneRow(SQLiteDatabase database, DatabaseRow row) {
        ...
        values.put(IndexDatabaseHelper.IndexColumns.DATA_SUMMARY_ON_NORMALIZED, row.normalizedSummaryOn);
        values.put(IndexDatabaseHelper.IndexColumns.DATA_SUMMARY_OFF, row.updatedSummaryOff);
        values.put(IndexDatabaseHelper.IndexColumns.DATA_SUMMARY_OFF_NORMALIZED, row.normalizedSummaryOff);
